...
class SRCDataModule(BaseDataModule):
    ...
    def setup(self, stage: Optional[str]):
        print("setup called")
        if stage == "fit":
            self.train_dataset = PTBDataset(self.data_type, "train", self.cfg)
            self.val_dataset = PTBDataset(self.data_type, "test", self.cfg)
        elif stage in ["test", "predict"]: # leave it for consistency. Although we are not using.
            self.test_dataset = PTBDataset(self.data_type, "test", self.cfg)
        else:
...
class SRCTRGDataModule(BaseDataModule):
    ...
    def setup(self, stage: Optional[str]):
        print("setup called")
        if stage == "fit":
            self.train_dataset_src = PTBDataset("src", "train", self.cfg)
            self.train_dataset_trg = PTBDataset("trg", "train", self.cfg)
            self.train_dataset = torch.utils.data.ConcatDataset([self.train_dataset_src, self.train_dataset_trg])
            self.val_dataset = PTBDataset("src", "test", self.cfg)
        elif stage in ["test", "predict"]:
            self.test_dataset = PTBDataset("trg", "test", self.cfg)
        else:
            raise ValueError(f"Unknown stage {stage}")
...
class PTBDataset(Dataset):
    ...
    def __init__(self, source, mode, cfg):
        ...
        self.cfg = cfg 
        self.source_ptb_data_name = self.cfg.data.da_data.src
        self.target_ptb_data_name = self.cfg.data.da_data.trg

        assert self.source_ptb_data_name > 0 and self.source_ptb_data_name <=4, "The source_ptb_data_name should be less than 5"
        assert self.target_ptb_data_name > 0 and self.target_ptb_data_name <=4, "The target_ptb_data_name should be less than 5"
        assert self.source_ptb_data_name != self.target_ptb_data_name, "The source_ptb_data_name and target_ptb_data_name should not be same"
        ...
        if self.source == "src":
            self.label_df = pd.read_csv(f"{base_dir}/DOMAIN{str(self.source_ptb_data_name).zfill(2)}_{mode}Y.csv")
            self.label_df['source'] = 1
            with open(f"{base_dir}/DOMAIN{str(self.source_ptb_data_name).zfill(2)}_{mode}X.pkl", "rb") as f:
                self.sensor_array = pickle.load(f)
            print(f"Loaded {self.source_ptb_data_name}_{mode}X.pkl and {self.source_ptb_data_name}_{mode}Y.csv")

        elif self.source == "trg":
            if mode in ["train", "test"]:
                self.label_df = pd.read_csv(f"{base_dir}/DOMAIN{str(self.target_ptb_data_name).zfill(2)}_{mode}Y.csv")
            else:
                raise ValueError(f"Unknown mode {mode}")
            
            self.label_df['source'] = 0
            with open(f"{base_dir}/DOMAIN{str(self.target_ptb_data_name).zfill(2)}_{mode}X.pkl", "rb") as f:
                self.sensor_array = pickle.load(f)
            print(f"Loaded {self.target_ptb_data_name}_{mode}X.pkl and {self.target_ptb_data_name}_{mode}Y.csv")
        else :
