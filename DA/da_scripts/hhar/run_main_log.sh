...
src_trg_pairs=("0 6" "1 6" "2 7" "3 8" "4 5" "5 0" "6 1" "7 4" "8 3" "0 2")
# src_trg_pairs=("2 7" "3 8")
pseudo_label_confidence_topk_sampling=True

feature_alignment=pot
gaussian_sim_sigma=0.1
save_cls_representations=True
use_distance_weighting=True
use_trg_class_prior=True
pseudo_topk_percent=0.2
prior_temperature=5.0

for use_trg_class_prior in true
do
    for use_distance_weighting in false true
    do
        for pair in "${src_trg_pairs[@]}"
        do
            read -r src trg <<< "$pair"
            echo "Start domain adaptation experiment $START_EXP_NUM, $src -> $trg, patch len: $patch_len,"
            CUDA_VISIBLE_DEVICES=$gpu_id python main.py data=$data data.da_data.src=$src  data.da_data.trg=$trg \
                                    ...
                                    da_model=$da_model \
                                    save_cls_representations=$save_cls_representations \
                                    ...
                                    likelihood.prior_temperature=$prior_temperature \
